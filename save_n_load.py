...
def load_all_backgrounds_chunky_type_separated(limit=1, detector="VD", bg_data_dir=None, adc_mode=None, sim_mode=None, offset=0):
    if adc_mode is None:
        adc_mode = ADC_MODE
    if sim_mode is None:
        sim_mode = SIM_MODE
...
def load_all_backgrounds_chunky(limit=1, detector="VD", type_separated=False):
    bg_total_hits = []
    bg_hit_list_per_event = []
...
def load_all_sn_events_chunky(limit=1, event_num=1000, detector="VD", sn_data_dir=None, adc_mode=None, sim_mode=None, offset=0):
...
def load_hit_data(file_name="pbg_g4_digi_reco_hist.root", event_num=-1, sn_event=False):
...
def load_neutrino_info(file_name):
    ...
    uproot_neutrino = uproot.open(file_name)["analysistree/anatree"]
    info_dict = uproot_neutrino.arrays(['enu_truth', 'nuvtxx_truth', 'nuvtxy_truth', 'nuvtxz_truth'], library="np")
